...
const categoryColors = {
    ...
    "病院": "#e74c3c",   // Red
    "障害": "#9b59b6",   // Purple
    "その他": "#f39c12"  // Orange (Changed to make the base location stand out)
};

// Create legend in Header
const legendContainer = document.getElementById('legend');
for (const [cat, color] of Object.entries(categoryColors)) {
    // Only show 居宅 and 基準地点 in legend
    if (cat !== "居宅" && cat !== "その他") continue;
    const displayName = cat === "その他" ? "基準地点" : cat;
    const item = document.createElement('div');
    item.className = 'legend-item';
    item.innerHTML = `<div class="legend-color" style="background-color: ${color}"></div><span>${displayName}</span>`;
...
plotMarkers(mapDataKawauchi5km);
